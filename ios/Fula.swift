...
class FulaModule: NSObject {
    ...
    func checkFailedActions(retry: Bool, timeout: Int, resolve: RCTPromiseResolveBlock, reject: RCTPromiseRejectBlock) -> Void {
        do {
            ...
            if !retry {
                ...
                if let nextFailedLink = try failedLinks.next() {
                    // Assuming nextFailedLink is of type Data
                ...
                let retryResults = try retryFailedActionsInternal(timeout: timeout)
    ...
    func createPeerIdentity(privateKey: Data) throws -> Data {
        ...
        if encryptedKey == nil {
            ...
            NSLog("ReactNative createPeerIdentity encryptedKey2=\(encryptedKey ?? "nil")")
    ...
    func loadWnfs(_ _wnfsKey: Data, _ _rootCid: String) throws {
        ...
        rootCid = _rootCid
        wnfsKey = _wnfsKey
        NSLog("ReactNative loadWnfs called: rootCid=\(rootCid ?? "nil")")
        if (self.fula != nil) {
            try self.fula?.flush()
        }
    ...
    func clearCidsFromRecent(cidArray: NSArray, resolve: @escaping RCTPromiseResolveBlock, reject: @escaping RCTPromiseRejectBlock) {
        // Convert NSArray to Swift Array immediately
        let swiftCidArray = cidArray as? [String] ?? []
        ...
        Task {
            do {
                ...
                // Use the Swift array directly
                let concatenatedCids = swiftCidArray.joined(separator: "|")
                ...
                try await fulaClient.clearCids(fromRecent: cidsData)
                resolve(true)
            } catch let error {
        ...
        DispatchQueue.global(qos: .default).async {
            do {
                ...
                while recentLinksIterator.hasNext() {
                    var error: NSError?
                    let nextLink = recentLinksIterator.next(&error)
                    ...
                    if let link = nextLink {
                        recentLinksList.append(link)
                    } else {
                        break // Exit the loop if nextLink is nil
                    }
                }
        ...
        Task {
            do {
                ...
                guard let poolID = Int64(poolIDStr) else {
                    throw MyError.runtimeError("Invalid poolID - not a valid number: \(poolIDStr)")
                }
                guard let replicationFactor = Int64(replicationFactorStr) else {
                    throw MyError.runtimeError("Invalid replicationFactor - not a valid number: \(replicationFactorStr)")
                }
                ...
                try await fulaClient.batchUploadManifest(cidsData, poolID: Int(poolID), replicationFactor: Int(replicationFactor))
                resolve(true)
            } catch let error {
